...
class MiniRotateProxy:
    """
    Minimal same-request rotation loop: on 429, mark auth as unavailable and retry with next auth.
    """
    ...
    def _do_call(self, auth: AuthRecord):
        ...
        if token and token.startswith("limited"):
            raise FakeHttpError(429)
        return "ok"


class SameRequestRotationTests(unittest.TestCase):
    def test_rotates_to_next_auth_on_429(self):
        ...
        auth1 = AuthRecord(
            id="a1",
            provider="anthropic",
            metadata={"access_token": "limited-1"},
            attributes={},
